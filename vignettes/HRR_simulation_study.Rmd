...
```{r loaddata, echo=F}
case1_res <- readRDS("HRR_simulation_files/RDS/case1_res.rds")
case2_res <- readRDS("HRR_simulation_files/RDS/case2_res.rds")
case3_res <- readRDS("HRR_simulation_files/RDS/case3_res.rds")
case4_res <- readRDS("HRR_simulation_files/RDS/case2_rev_res.rds")
```
...
## Case1 
`case1_fnc` generates data in uniform distribution and mix it with cluster data.
...
TP <- sum(case1_mtx[,3])/length(case1_mtx[,3])
FP <- 1-TP
#------------------------------------ plot ------------------------------------#
...
p_clst <- ggplot(data.frame(x = seq(0,1,by = 0.001)),mapping = aes(x)) + 
  stat_function(fun = function(x)sapply(x,cnt_fnc)) +
  ylim(c(0,sim_rep)) +
  ggtitle("Detected Clusters") +
...
## Case2
`case2_fnc` generates data with fourier first order density function and mix it with cluster data.
The function returns a matrix of 3 columns: start,end,if detected cluster intersects true cluster
```{r case2_fnc}
...
TP <- sum(case2_mtx[,3])/length(case2_mtx[,3])
FP <- 1-TP
#------------------------------------ plot ------------------------------------#
## cumulative detected clusters ##
...
## Case3
`case3_fnc` generates data with fourier first order density function and mix it with cluster data.
The function returns a matrix of 3 columns: start,end,if detected cluster intersects true cluster
```{r case3_fnc}
...
## scan results ##
case3_res <- mclapply(seq.int(sim_rep), case3_fnc)
```
...
TP <- sum(case3_mtx[,3])/length(case3_mtx[,3])
FP <- 1-TP
#------------------------------------ plot ------------------------------------#
## cumulative detected clusters ##
...
## plot histogram ##
y_base <- POT::qgpd(x_base, loc = mu, scale = sig, shape = xi)
y_clst <- qgpd(x_clst, loc = mu, scale = sig, shape = xi)
y_all <- sort(c(y_clst,y_base))

ggplot(data.frame(x = y_all[y_all<100]), mapping = aes(x)) + 
  geom_histogram(binwidth = 2)
```
...
## scan results ##
case4_res <- mclapply(seq.int(sim_rep), case2_fnc)
```

```{r rev_case2_result}
## true positive and false positive ##
case4_mtx <- do.call(rbind, case4_res)

TP <- sum(case4_mtx[,3])/length(case4_mtx[,3])
FP <- 1-TP
#------------------------------------ plot ------------------------------------#
## cumulative detected clusters ##
cnt_fnc <- function(x) sum(case4_mtx[,1] <= x & case4_mtx[,2] >= x)
...
## x_all ##
x_all <- POT::qgpd(cdf_x, loc = mu, scale = sig, shape = xi, lower.tail = F)
p_hist <- ggplot(data.frame(x = x_all[x_all<100]), mapping = aes(x)) + 
  geom_histogram(binwidth = 2) + 
  ggtitle("Example of gp+cluster") +
  theme(plot.title = element_text(hjust = 0.5))
