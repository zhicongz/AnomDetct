...
model comes with default parameters. This the reason this method is decomposed 
...
if(requireNamespace(POT, quietly = T)){
  foo <- AnomDetct::tran_detct(x = AnomDetct::sim_df_N, loc = 10)
  foo$Plot
}
```
...
Length of 1-dependent sequence is depends on `length(x)/window_lth`. This 
error mostly occurs with too large `window_lth` especially when `window_lth` 
is selected by `AnomDetct::MLE_window_lth()`. In this case, users can try 
manually input `window_lth` or `AnomDetct::Haiman_window_lth()`.

### inappropriate `theta_th`

```{r, error = T, purl = F}
foo <- AnomDetct::ultimate_detct(x = AnomDetct::sim_df_N, theta_th = 10)
foo$Plot
```

Even though the function `AnomDetct::ultimate_detct()` only requests `theta_th` 
to be positive, too large `theta_th` leads to too much false positive clusters 
and unsteatable test statistics distribution approximations.
